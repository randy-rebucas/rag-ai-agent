...
const shopify = shopifyApp({
  ...
  hooks: {
    afterAuth: async ({ session }) => {
      ...
      if (shopRecord.initialSyncDone) return;

      const { admin } = await shopify.unauthenticated.admin(session.shop);
      // Fire-and-forget: bulk sync can take minutes, don't block the OAuth redirect.
      runInitialSync(shopRecord.id, admin).catch((error) => {
        console.error(`Initial sync failed for ${session.shop}:`, error);
      });
    },
  },
